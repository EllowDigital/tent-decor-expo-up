...
function ExhibitorsPage() {
  ...
  const stallTypes = [
    ...
    },
    {
      ...
      includes: ["Raw floor space", "Custom build allowed", "Prime aisle options", "Overnight security"],
    },
    {
      ...
      includes: ["Anchor hall placement", "Hospitality lounge", "Priority signage", "Awards submission"],
    },
  ];

  const process = [
    { icon: FileText, title: "Submit enquiry", desc: "Share stall size, product category and preferred hall." },
    { icon: Handshake, title: "Team callback", desc: "Our exhibitor desk confirms availability within 24 business hours." },
    { icon: Store, title: "Reserve & pay", desc: "Confirm the stall with an advance and receive floor-plan proof." },
    { icon: Trophy, title: "Show up & sell", desc: "Move-in one day prior. Meet buyers across three focused days." },
  ];
  ...
  return (
    <>
          ...
          <div className="grid lg:grid-cols-12 gap-8 lg:gap-12 items-end">
            <div className="lg:col-span-7 min-w-0">
              ...
              <p className="mt-5 max-w-xl text-white/70 text-base sm:text-lg leading-relaxed">
                Meet decision-makers, launch products, and close orders on the
                floor. Choose a shell scheme or design a bespoke pavilion.
              </p>
            ...
            <div className="lg:col-span-5">
              <div className="rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-sm p-5 sm:p-6">
                ...
                <dl className="mt-5 space-y-3.5">
                  ...
                  <MetaRow icon={Users} label="Audience" value={`${upcoming.exhibitors} exhibitors · ${upcoming.visitors} visitors`} />
                </dl>
              </div>
            </div>
          </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="max-w-2xl">
            ...
            <p className="mt-3 text-slate-muted">
              Pick the right footprint for your brand. We'll handle logistics —
            ...
            {stallTypes.map((s) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`h-10 w-10 rounded-lg grid place-items-center ${featured ? "bg-gold/15" : "bg-gold/10"}`}>
                    <Store className="h-5 w-5 text-gold" />
                  </span>
                  <h3 className={`mt-5 font-display text-xl font-semibold ${featured ? "text-white" : "text-charcoal"}`}>
                    {s.title}
                  </h3>
                  <p className={`mt-1 text-sm ${featured ? "text-gold" : "text-gold/80"} font-medium`}>{s.size}</p>
                  <p className={`mt-3 text-sm leading-relaxed ${featured ? "text-white/70" : "text-slate-muted"}`}>
                    {s.desc}
                  </p>
                  <ul className={`mt-5 space-y-2 text-sm ${featured ? "text-white/85" : "text-charcoal/80"}`}>
                    {s.includes.map((i) => (
          ...
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-4 text-slate-muted leading-relaxed">
                Shell fabrication, utility connections, hospitality — handled.
                You show up, plug in, and sell.
              </p>
      ...
      <section className="py-16 sm:py-20 lg:py-24 bg-white">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="max-w-2xl">
            <span className="text-[11px] uppercase tracking-[0.32em] text-gold font-medium">Booking process</span>
            <h2 className="mt-3 font-display font-bold text-charcoal text-[clamp(1.75rem,4vw,2.75rem)] leading-tight">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-10 grid gap-3 sm:gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {EXHIBITOR_CATEGORIES.map((c) => (
              <div key={c.title} className="rounded-xl border border-border/60 bg-white p-5 transition-all hover:border-gold/50 hover:shadow-lg">
